...
### Exercise 2 - Answer about `bed_slop()`

**The `bed_slop()` function allows you to change the size of input intervals (i.e., you could increase the size of your interval by 1000 nucleotides on both sides of a transcription start site). Therefore, it requires the lengths of each chromosome to ensure that the size of input intervals is within bounds of the chromosomes.**

```{r answer_2}
slop_bed <- bed_slop(tss, genome, both = 1000) 
...
```{r map_glyph, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Imagine that each X interval is a TSS interval and the Y intervals are the Pol2 ChipSeq bedgraph intervals. The numbers 12, 20, 30, etc. indicate the  Chip-Seq coverage across each y interval. What bed_map() will do is find all intervals in y that overlap each x interval, then allow you to compute a summary statistic, in this case a sum. Conceptually this is similar to the group_by() and summarize() functions in dplyr, but instead of grouping by columns we are grouping y intervals that overlap the x intervals. If there are no Y intervals overlaping an X interval then an NA value is reported."}
...
```{r answer_6, fig.cap = "Figure for Answer 6. Metagene profile  of RNA PolII coverage surrounding the transcription start site, +/- 1000 nucleotides. The peak of RNA PolII coverage correlates to the TSS, which is not surprising considering that RNA PolII binds to the TSS to initiate transcription of DNA."}
region_size <- 1000
win_size <- 10
...
group_by(chip_coverage, .win_id) %>% 
  summarize(.win_id_mean = mean(total_reads, na.rm = TRUE)) %>% 
  ggplot(., aes(.win_id, .win_id_mean)) +
  geom_point() +
  scale_x_continuous(labels = x_labels, breaks = x_breaks) +
  labs(title = "Metagene across all human Transcription Start Sites (TSS)",
      x = "Nucleotides from TSS",
      y = "Mean RNA PolII Coverage (RPM)")
```
